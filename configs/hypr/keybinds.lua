...
hl.bind(mainMod .. " + F", hl.dsp.exec_cmd("helium-browser"))
...
hl.bind("CTRL + SHIFT + S",
    hl.dsp.exec_cmd('grim -g "$(slurp)" - | tee ~/Pictures/$(date +\'%Y-%m-%d_%H-%M-%S.png\') | wl-copy'))

-- GESTIÓN DE VENTANAS
hl.bind(mainMod .. " + C", hl.dsp.window.close())
hl.bind(mainMod .. " + V", hl.dsp.window.float({ action = "toggle" }))
hl.bind(mainMod .. " + P", hl.dsp.window.fullscreen({ mode = 0 }))
-- hl.bind("SUPER", "C", "exec", [[ sh -c 'if [ "$(hyprctl -j getoption general:layout | jq -r .str)" = "dwindle" ]; then hyprctl keyword general:layout master; else hyprctl keyword general:layout dwindle; fi' ]])
